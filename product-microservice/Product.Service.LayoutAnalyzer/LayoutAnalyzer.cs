...
namespace Product.Service.LayoutAnalyzer;

[DiagnosticAnalyzer(LanguageNames.CSharp)]
public sealed class LayoutAnalyzer : DiagnosticAnalyzer
{
    public const string DomainRuleId = "PRDLAY001";
    public const string FeatureSliceRuleId = "PRDLAY002";
    public const string InfrastructureRuleId = "PRDLAY003";
    public const string ContractsRuleId = "PRDLAY004";
    ...
    private static void AnalyzeSyntaxTree(SyntaxTreeAnalysisContext context)
    {
        ...
        if (fileNamespace is null || !StartsWith(fileNamespace, "Product.Service"))
        {
            return;
        }
        ...
        foreach (var usingDirective in root.DescendantNodes().OfType<UsingDirectiveSyntax>())
        {
            ...
            if (target is null || !StartsWith(target, "Product.Service"))
            {
                continue;
            }
        ...
        string targetNamespace)
    {
        ...
        if (StartsWith(fileNamespace, "Product.Service.Domain")
            && (StartsWith(targetNamespace, "Product.Service.Infrastructure")
        ...
        if (StartsWith(fileNamespace, "Product.Service.Infrastructure")
            && StartsWith(targetNamespace, "Product.Service.Features"))
        {
        ...
        if (StartsWith(fileNamespace, "Product.Service.Contracts")
            && (StartsWith(targetNamespace, "Product.Service.Domain")
                || StartsWith(targetNamespace, "Product.Service.Infrastructure")
                || StartsWith(targetNamespace, "Product.Service.Features")
                || StartsWith(targetNamespace, "Product.Service.Endpoints")
                || StartsWith(targetNamespace, "Product.Service.ApiModels")
                || StartsWith(targetNamespace, "Product.Service.IntegrationEvents")))
        {
    ...
    {
        const string prefix = "Product.Service.Features.";
