...
1. **Two saga aggregates coexist in one service** (`OrderSagaState`, `RefundSagaState`) with mostly disjoint inbound triggers but one shared inbound (`PaymentRefundedEvent`). The current layout flattens both sagas into the same folders, so "is this for Order or Refund?" is answerable only by reading the handler body.
...
`PaymentRefundedEvent` (the one event consumed by both sagas) is handled by registering **two** slice handlers: `Features/OrderSaga/PaymentRefunded/` AND `Features/RefundSaga/PaymentRefunded/`. The `ECommerce.Shared` event bus dispatches the message to both handlers; each loads its own saga state by id and no-ops if the message does not belong to its saga. This costs one extra store lookup per refund but keeps the "one inbound trigger per slice" rule intact and avoids slice-to-slice references.
...
11. As a Saga service developer, I want `PaymentRefundedEvent` to be handled by two distinct slices (`Features/OrderSaga/PaymentRefunded/` and `Features/RefundSaga/PaymentRefunded/`), each registered as `IEventHandler<PaymentRefundedEvent>`, each looking up its own saga state by id and no-oping if the message is not its own, so that "one inbound trigger per slice" stays true and no cross-slice reference is introduced.
...
28. As an architect, I want the CLAUDE.md entry to explicitly call out saga-specific divergences vs prior pilots (two-level `Features/<Saga>/<Trigger>/` namespace nesting; `ISagaTransitionRunner<TState, TEvent>` Domain abstraction new to saga; reply-processor fan-out routers dissolved; no `IIntegrationMap<,>` seam; dual-subscription convention for `PaymentRefundedEvent`; reaper as `Infrastructure/Reaper/` hosted service mirroring Shipping's `CarrierPollingService`; absence of any HTTP write endpoint outside `Features/Operator/AbortSaga/`), so that future contributors understand why saga looks slightly different.
...
- `Features/<Saga>/<Trigger>/` — two-level nesting under `Features/`. Final slice list (19 total):
  ...
  - **RefundSaga (2 slices):** `RefundRequested/`, `PaymentRefunded/`.
...
- Two slices register as `IEventHandler<PaymentRefundedEvent>`: `Features/OrderSaga/PaymentRefunded/` and `Features/RefundSaga/PaymentRefunded/`.
- `ECommerce.Shared` event bus dispatches the message to both handlers.
- Each handler attempts to load its saga state by id (`OrderSagaState` by `evt.OrderId`; `RefundSagaState` by `evt.RefundId`) and no-ops if the message does not belong to its saga.
...
- NetArchTest rule forbids `Saga.Service.Features.X.*` referencing `Saga.Service.Features.Y.*` for any distinct slices, including across `OrderSaga` / `RefundSaga` / `Operator`. The dual-subscription `PaymentRefunded` slices each stand alone — neither references the other.
...
- Staged commits land in this order, each green:
  ...
  7. RefundSaga slices: extract `RefundRequested/` + `PaymentRefunded/`. Delete `RefundSagaReplyProcessor`.
...
- **Dual-subscription dispatch for `PaymentRefundedEvent`** — new integration test asserting both `Features/OrderSaga/PaymentRefunded/Handler` and `Features/RefundSaga/PaymentRefunded/Handler` are invoked when the event is published, and each no-ops if its saga is not found.
...
- `PaymentRefundedEvent` dual-subscription is the only place in the monorepo where a single integration event is consumed by two slices that must both act on it (rather than one slice routing based on payload). The two-handler model is preferable to a router because the alternative requires a slice that references both saga slices, which the NetArchTest rule forbids. The cost (one extra store lookup per refund) is bounded and observable.
