...
cluster (Docker Desktop or Minikube) using the manifests in [`kubernetes/`](https://github.com/daonhan/Microservices-in-.NET/tree/main/kubernetes). The goal is
to validate manifest changes before they hit AKS — same shapes, same probes, same DNS
naming conventions, but on your laptop.

> Source-of-truth copy lives at [`docs/LOCAL_K8S_GUIDE.md`](https://github.com/daonhan/Microservices-in-.NET/blob/main/docs/LOCAL_K8S_GUIDE.md). For the AKS / cloud flow, see [Azure-Deployment](Azure-Deployment); for the basic apply order, see [Kubernetes-Deployment](Kubernetes-Deployment).
...
crash-loop while infra is still coming up. Use the wrappers in [`scripts/`](https://github.com/daonhan/Microservices-in-.NET/tree/main/scripts):
...
The manifests wire `/health/ready` and `/health/live` (see [Shared-Library](Shared-Library)).
Hit them through `kubectl exec` to bypass `LoadBalancer` quirks on local clusters:
...
[`UserConfiguration.cs`](https://github.com/daonhan/Microservices-in-.NET/blob/main/auth-microservice/Auth.Service/Infrastructure/Data/EntityFramework/Configurations/UserConfiguration.cs)),
...
[`api-gateway/ApiGateway/appsettings.json`](https://github.com/daonhan/Microservices-in-.NET/blob/main/api-gateway/ApiGateway/appsettings.json)
...
**Verified fix on Docker Desktop.** [`kubernetes/sql.yaml`](https://github.com/daonhan/Microservices-in-.NET/blob/main/kubernetes/sql.yaml)
...
Both [`scripts/apply-local-k8s.ps1`](https://github.com/daonhan/Microservices-in-.NET/blob/main/scripts/apply-local-k8s.ps1) and
[`scripts/apply-local-k8s.sh`](https://github.com/daonhan/Microservices-in-.NET/blob/main/scripts/apply-local-k8s.sh) detect this
...
Then edit [`kubernetes/sql.yaml`](https://github.com/daonhan/Microservices-in-.NET/blob/main/kubernetes/sql.yaml) to use `ReadWriteOnce`:
...
- Once you are comfortable here, the AKS Dev/Staging/Prod manifests live alongside
  the local ones in [`kubernetes/`](https://github.com/daonhan/Microservices-in-.NET/tree/main/kubernetes) under the `aks-{env}-*.yml`
...
- AKS-specific deltas (HPA, Application Insights, ACR image pulls, namespaces,
  ...
  [`kubernetes/product-microservice.yaml`](https://github.com/daonhan/Microservices-in-.NET/blob/main/kubernetes/product-microservice.yaml))
