...
Zero behavior change. Every existing `Order.Tests` test must continue to pass without modification beyond namespace renames. `ECommerce.Shared` is untouched (no nupkg version bump). No new EF migrations. No changes to public HTTP routes, response shapes, status codes, auth requirements, integration event payload contracts, or outbox / DLQ behavior.
...
- **Shared library**: `ECommerce.Shared` is not modified. No `dotnet pack`, no nupkg version bump. The pilot composes existing `AddSqlServerDatastore`, `AddOutbox`, `AddPlatformEventBus`, `AddEventHandler`, `AddPlatformObservability`, `AddPlatformHealthChecks`, `AddPlatformOpenApi`, `AuthorizationPolicies.RequireServicePolicy`.
