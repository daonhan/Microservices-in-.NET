...
namespace Auth.Service.LayoutAnalyzer;

[DiagnosticAnalyzer(LanguageNames.CSharp)]
public sealed class LayoutAnalyzer : DiagnosticAnalyzer
{
    public const string DomainRuleId = "AUTLAY001";
    public const string FeatureSliceRuleId = "AUTLAY002";
    public const string InfrastructureRuleId = "AUTLAY003";
        ...
        string targetNamespace)
    {
        ...
        if (StartsWith(fileNamespace, "Auth.Service.Domain")
            && (StartsWith(targetNamespace, "Auth.Service.Infrastructure")
                || StartsWith(targetNamespace, "Auth.Service.Features")))
        {
            context.ReportDiagnostic(Diagnostic.Create(DomainRule, location, fileNamespace, targetNamespace));
            return;
        }

        if (StartsWith(fileNamespace, "Auth.Service.Features")
            && StartsWith(targetNamespace, "Auth.Service.Features"))
        {
        ...
        if (StartsWith(fileNamespace, "Auth.Service.Infrastructure")
            && StartsWith(targetNamespace, "Auth.Service.Features"))
        {
    ...
    {
        const string prefix = "Auth.Service.Features.";
