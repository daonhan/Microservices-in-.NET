...
2. **Latent eager-broker defect carried by older pins.** The lazy `IRabbitMqConnection` registration fix shipped in `2.24.0` (commit `dcbc29c`). Auth, Basket, and Product still pin pre-fix versions and silently carry the eager defect; Auth, Saga, and ApiGateway are additionally the three consumers without a `MessagingProviderBootTests`-equivalent gate that would surface a regression at test time (see [`docs/runbooks/shared-libs-versioning.md`](../runbooks/shared-libs-versioning.md) §"Per-PR loop").
