...
- **Constants stay local.** The Operator persona constants live in the auth seeder; the fixture GUIDs/values live in the gateway seeder. They are **not** promoted into shared `QaPersonas`, which would force a shared-libs `<Version>` bump + repack + consumer re-pin (see [ADR-0013](0013-shared-libs-multi-package-split.md)) disproportionate to a smoke fixture. The Postman/Bruno env files remain the black-box source of truth; the lockstep invariant (values agree across `qa/postman/qa-local.postman_environment.json`, `qa/bruno/environments/qa-local.bru`, `qa/postman/AGENT.md`, and `scripts/local-smoke-test.ps1 $Qa`) is unchanged.
