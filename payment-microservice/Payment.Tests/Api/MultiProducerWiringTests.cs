...
// Pins the multi-producer wiring contract: HTTP slices and saga command slices both raise the
// same domain events and must route through a single PaymentCapturedIntegrationMap /
...
public class MultiProducerWiringTests : IntegrationTestBase
{
    public MultiProducerWiringTests(PaymentWebApplicationFactory factory)
        : base(factory)
    {
    }
