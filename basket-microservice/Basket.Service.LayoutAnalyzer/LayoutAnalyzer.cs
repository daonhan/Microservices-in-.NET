...
namespace Basket.Service.LayoutAnalyzer;

[DiagnosticAnalyzer(LanguageNames.CSharp)]
public sealed class LayoutAnalyzer : DiagnosticAnalyzer
{
    public const string DomainRuleId = "BSKLAY001";
    public const string FeatureSliceRuleId = "BSKLAY002";
    public const string InfrastructureRuleId = "BSKLAY003";
    public const string ContractsRuleId = "BSKLAY004";
    ...
    private static readonly DiagnosticDescriptor ContractsRule = new(
        ContractsRuleId,
        "Contracts may not reference any other internal Basket.Service.* namespace",
    ...
    private static void AnalyzeSyntaxTree(SyntaxTreeAnalysisContext context)
    {
        ...
        if (fileNamespace is null || !StartsWith(fileNamespace, "Basket.Service"))
        {
            return;
        }
        ...
        foreach (var usingDirective in root.DescendantNodes().OfType<UsingDirectiveSyntax>())
        {
            ...
            if (target is null || !StartsWith(target, "Basket.Service"))
            {
                continue;
            }
        ...
        string targetNamespace)
    {
        ...
        if (StartsWith(fileNamespace, "Basket.Service.Domain")
            && (StartsWith(targetNamespace, "Basket.Service.Infrastructure")
                || StartsWith(targetNamespace, "Basket.Service.Features")))
        {
            context.ReportDiagnostic(Diagnostic.Create(DomainRule, location, fileNamespace, targetNamespace));
            return;
        }

        if (StartsWith(fileNamespace, "Basket.Service.Features")
            && StartsWith(targetNamespace, "Basket.Service.Features"))
        {
        ...
        if (StartsWith(fileNamespace, "Basket.Service.Infrastructure")
            && StartsWith(targetNamespace, "Basket.Service.Features"))
        {
        ...
        if (StartsWith(fileNamespace, "Basket.Service.Contracts")
            && (StartsWith(targetNamespace, "Basket.Service.Domain")
                || StartsWith(targetNamespace, "Basket.Service.Infrastructure")
                || StartsWith(targetNamespace, "Basket.Service.Features")
                || StartsWith(targetNamespace, "Basket.Service.Endpoints")
                || StartsWith(targetNamespace, "Basket.Service.ApiModels")
                || StartsWith(targetNamespace, "Basket.Service.IntegrationEvents")))
        {
    ...
    {
        const string prefix = "Basket.Service.Features.";
